/* PlayStation(R)Mobile SDK 1.21.01
 * Copyright (C) 2013 Sony Computer Entertainment Inc.
 * All Rights Reserved.
 */
using System;
using System.Diagnostics;
using Sce.PlayStation.Core;
using Sce.PlayStation.Core.Audio;
using Sce.PlayStation.Core.Graphics;
using DemoGame;

namespace Physics2dDemo
{

///
/// SceneResultクラス
///
public class SceneResult : Scene
{
	private int current_scene; 
	private LayoutAction action;
	private bool isPress = false;

    /// コンストラクタ
    public SceneResult()
    {
    }

    /// デストラクタ
    ~SceneResult()
    {
    }
	
    /// シーンの初期化
	///
	/// @param [out]
	///
    public override bool Start()
    {
		isPress = false;
		LayoutAnimationList animList = new LayoutAnimationList();
		current_scene = GameData.StageNum%3; 
		action = new LayoutAction();
		if(current_scene == 0){
			var image = Resource2d.GetInstance().ImageLyt_ecB;
			animList.Add(new SpriteAnimation(new Sprite(image,0,0,854,480,0,0),
				0, 0, false, 0, 0, 0xff, 0, 0, 0xff));
		}
		else if(current_scene == 1){
			var image = Resource2d.GetInstance().ImageLyt_ecC;
			animList.Add(new SpriteAnimation(new Sprite(image,0,0,854,480,0,0),
				0, 0, false, 0, 0, 0xff, 0, 0, 0xff));
		}
		else if(current_scene == 2){
			var image = Resource2d.GetInstance().ImageLyt_ecD;
			animList.Add(new SpriteAnimation(new Sprite(image,0,0,854,480,0,0),
				0, 0, false, 0, 0, 0xff, 0, 0, 0xff));
		}
		
		action.Add("BG", animList);
		action.SetCurrent("BG");

        AudioManager.AddBgm("Stage", "/Application/res/data/snd/S42.mp3");
        AudioManager.AddSound("Press", "/Application/res/data/snd/S01.wav");
			
		AudioManager.PlayBgm("Stage");

		return true;
	}
	
    /// シーンの破棄
    public override void End()
    {
		Graphics2D.ClearSprite();
		
		if(action != null)
		{
			action.Dispose();
			action = null;
		}

		AudioManager.Clear();
	}
	
    /// フレーム処理
	///
	/// @param [out]
	///
    public override bool Frame()
    {
		//遷移後は処理しない
		if(current_scene != GameData.StageNum%3)
			return true;

        var pad   = InputManager.InputGamePad;
        var touch = InputManager.InputTouch;
		if ( pad.Trig != 0 || touch.GetInputNum() > 0 ) {
			int scene = GameData.StageNum%3; 
			// ステージ名表示
			switch(scene)
			{
			case 0:
				GameData.StageNum++;
				SetNextScene(new SceneMain());
				break;
			case 1:
				GameData.StageNum++;
				SetNextScene(new SceneMain());
				break;
			case 2:
				GameData.StageNum++;
				if(GameData.StageNum < 6)
					SetNextScene(new SceneStart());
				else
					SetNextScene(new SceneTitle());
					
				break;
			}
            if(!isPress)
				AudioManager.PlaySound("Press");
			isPress = true;
			return true;
		}
		
		return true;
	}
	
    /// 描画
	///
	/// @param [out]
	///
    public override bool Draw()
    {
		action.Render();

		return true;
	}

}
} // Physics2dDemo
