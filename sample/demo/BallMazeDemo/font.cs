...
namespace Demo_BallMaze
{
	...
	class TextPlane : IDisposable
	{
		...
		private static GraphicsContext	graphics = BallMazeProgram.graphics;
		...
		// コンストラクタ
		public TextPlane(int heightSmall, int heightLarge)
		{
			...
			fontSmall = new Font(FontAlias.System, heightSmall, FontStyle.Regular);
			fontLarge = new Font(FontAlias.System, heightLarge, FontStyle.Regular);
		...
		public void Print(ImagePosition pos, ImageColor color, string text, bool bDropShadow, bool bUseLarge, int id, bool isUpdate)
		{
			int	 listID = -1;
			...
			if (bUseLarge)
			{
				if (pos.X == -1)
				{
					pos.X = (Defs.SCREEN_WIDTH - fontLarge.GetTextWidth(text)) / 2;
				}
				addFont = fontLarge;
			}
			else
			{
				...
				if (pos.X == -1)
				{
					pos.X = (Defs.SCREEN_WIDTH - fontSmall.GetTextWidth(text)) / 2;
				}
				addFont = fontSmall;
			}
			...
			for(int i=0; i < textDict.Count; i++){
				...
				if(listID == id){
					if(true == listIsUpdate){
						textDict.RemoveAt(i);
						textDict.Add(new TextData(addFont, pos, color, text, id, isUpdate));
					}
					return;
				}
			}
