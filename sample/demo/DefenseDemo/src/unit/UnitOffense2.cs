	...
	: Unit
{
	...
	public override bool Init()
	{
		...
		obj3d[OBJ3D_ENE02L2] = resContainer.GetObject3D( "ENE02L2" );
		...
		type = Unit.TYPE_ACT;
		/// ユニット種別の設定
		kind = Unit.KIND_ACT_ARMORED_CAR;
		/// ユニットHPを取得
		...
		animIndex[OBJ3D_ENE02L2] = 0;
		animFrame[OBJ3D_ENE02L2] = 0.0f;
	...
	public override void Update()
	{
		...
		if( changeModelLevel0 || changeModelLevel2 ){
			...
			if( changeModelLevel0 && modelLevel != 0 ){
				obj3d[0] = resContainer.GetObject3D( "ENE02L0" );
			...
			if( changeModelLevel2 && modelLevel != 2 ){
				obj3d[0] = resContainer.GetObject3D( "ENE02L2" );
		...
		if( !deadEffectStart  ){
			...
			if( animIndex[OBJ3D_ENE02L2] == 0 ){
				if( (float)((float)hp/(float)Unit.ACT_PARAM_DATA[(kind*Unit.ACT_PARAM_NUM)+Unit.ACT_PARAM_HP]) <= 0.5f ){
					animIndex[OBJ3D_ENE02L2] = 1;
					animFrame[OBJ3D_ENE02L2] = 0.0f;
				}
			}
			else if( animIndex[OBJ3D_ENE02L2] == 1 ){
				if( (float)((float)hp/(float)Unit.ACT_PARAM_DATA[(kind*Unit.ACT_PARAM_NUM)+Unit.ACT_PARAM_HP]) <= 0.25f ){
					animIndex[OBJ3D_ENE02L2] = 2;
					animFrame[OBJ3D_ENE02L2] = 0.0f;
				}
			}

			animFrame[OBJ3D_ENE02L2]++;
			if( animFrame[OBJ3D_ENE02L2] >= obj3d[OBJ3D_ENE02L2].model.GetMotionFrameMax( animIndex[OBJ3D_ENE02L2] ) ){
				animFrame[OBJ3D_ENE02L2] = 0.0f;
			}
		}
	}
	...
	public override void Render()
	{
		...
		obj3d[OBJ3D_ENE02L2].model.WorldMatrix = drawTmpPosture.GetPosture();
		obj3d[OBJ3D_ENE02L2].model.SetAnimFrame( animIndex[OBJ3D_ENE02L2], animFrame[OBJ3D_ENE02L2] );
        obj3d[OBJ3D_ENE02L2].model.Update();
        obj3d[OBJ3D_ENE02L2].model.Draw( graphics, resContainer.shaderContainer, (camInfo.GetProjection()*camInfo.GetView().Inverse()), camInfo.GetViewEye() );
	}
