	...
	static bool Render()
	{
		...
		if ( workers == null ) {
			...
			for ( int N = 0 ; N < workerCount ; N ++ ) {
				int start = spriteCount * N / workerCount ;
				int end = spriteCount * ( N + 1 ) / workerCount ;
				Action action = () => {
					for ( int i = start ; i < end ; i ++ ) {
						MoveSprite( sprites[ i ], SampleTimer.DeltaTime ) ;
					}
				} ;
				bool now = ( N == workerCount - 1 ) ;
				workers[ N ].Start( action, now ) ;
			}
			ThreadWorker.WaitAll( workers ) ;
		}
		batch.Draw() ;

		SampleDraw.DrawText( "Sprite Thread Sample", 0xffffffff, 0, 0 ) ;
