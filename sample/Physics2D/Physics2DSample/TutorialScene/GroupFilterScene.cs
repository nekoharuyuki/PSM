...
//
// The way to use group filter property of the rigid body
//
...
// [Tips and notice]
//
// The operation "&" for groupFilter of two rigid sceneBodies are not equals to zero,
// two rigid sceneBodies are proposed to join the same kind of group and the collision detection is enabled.
//
// [Physics2DEngine/Broadphase.cs]
// if ((ob1.groupFilter & ob2.groupFilter) == 0) return false;  (cancel collision detection if equals to zero)
//
...
namespace Physics2DSample
{
    ...
    public class GroupFilterScene : PhysicsScene
	{	
		...
		public override void InitScene ()
        {
			...
			// Create a box with group filter ( 1 << 1 )
			for(int i=0; i<2; i++)
			{
			...
			// Create a sphere with group filter ( 1 << 2)
			for(int i=0; i<2; i++)
			{
			...
			// Create a sphere with group filter ( 1<<1 | 1 << 3)
			for(int i=0; i<2; i++)
			{
            ...
            for (int j = 0; j < numShape; j++)
            {
                ...
                for (int i = 0; i < numBody; i++)
                {
                    ...
                    else
                    {
						...
						switch(sceneBodies[i].groupFilter)
						{
							case (1<<1):
