...
//
// SoftBody creation by joint setting
//
...
// [Tips and notice]
//
// Calculation cost of joint solver is not small compared with spring feature
//
// When there are few soft bodies, then it will possible to create those softbodies by joint setting.
// But please check the performance on actual devices.
//
// As this sasmple show, axis1Lim, axis2Lim and angleLim of the joint can change how strong or how weak joint can behave.
//
//
...
namespace Physics2DSample
{
    ...
    public class JointSoftbodyScene : PhysicsScene
	{	
		...
		public override void InitScene ()
        {
            ...
            base.InitScene();
			sceneName = "JointSoftbodyScene";
        
			// joint calculation is not small
			// should check whether the performance is acceptable on mobile devices
			printPerf = true;
            ...
            Vector2 box_width0 = new Vector2(2.0f, 2.0f);
            sceneShapes[3] = new PhysicsShape(box_width0);		
            
			float sphere_width2 = 2.0f;
			sceneShapes[4] = new PhysicsShape(sphere_width2);
			...
			for(int i=0; i<3; i++)
			{
				...
				{			
					if(i==2)
					{
						sceneBodies[numBody] = new PhysicsBody(sceneShapes[2], 0.2f); 
						sceneBodies[numBody].position = new Vector2(20.0f + 5.0f*j, 0.0f + 5.0f*i);
					...
					if(j!=0)
					{
						...
						sceneJoints[numJoint].axis1Lim = new Vector2(strongness, 0.0f);
						sceneJoints[numJoint].axis2Lim = new Vector2(0.0f, strongness);
						sceneJoints[numJoint].angleLim = strongness;	
						numJoint++;
					}
					
					if(i!=0)
					{
						PhysicsBody bodyA = sceneBodies[numBody-4];
						PhysicsBody bodyB = sceneBodies[numBody-1];
						sceneJoints[numJoint] = new PhysicsJoint(bodyA, bodyB, 0.5f*(bodyA.position + bodyB.position), (uint)numBody-4, (uint)numBody-1);
						sceneJoints[numJoint].axis1Lim = new Vector2(strongness, 0.0f);
						sceneJoints[numJoint].axis2Lim = new Vector2(0.0f, strongness);
						sceneJoints[numJoint].angleLim = strongness;
						numJoint++;
		...
		public override void InitScene ()
        {
			...
			for(int i=0; i<3; i++)
			{
				for(int j=0; j<3; j++)
				{				
					PhysicsBody body = AddBody(SceneShapes[2], 0.2f); 
					body.Position = new Vector2(20.0f + 5.0f*j, 0.0f + 5.0f*i);
					...
					if(j!=0)
					{
						...
						joint.Axis1Lim = new Vector2(strongness, 0.0f);
						joint.Axis2Lim = new Vector2(0.0f, strongness);
						joint.AngleLim = strongness;	
					}
