...
//
// Creation of a slider joint for any directions
//
...
namespace Physics2DSample
{
    ...
    public class AnyDirSliderScene : PhysicsScene
	{	
		...
		public override void InitScene ()
        {
            ...
            // Any Direction Slider Joint without distance limitation
            {
				sceneBodies[numBody] = new PhysicsBody(sceneShapes[2], 1.0f);
				sceneBodies[numBody].position = new Vector2(-15.0f, 0.0f);
				...
				// (Joint is more stable if you choose a relatively bigger static rigid body for creating the joint)
				//
				...
				// Lock the movement for the direction = nomalization of Vector2(-1.0f, 2.0f)
				// It means that the movement is free for the direction = nomalization of Vector2(2.0f, 1.0f)
			...
			// Any Direction Slider Joint with distance limitation
            {
				sceneBodies[numBody] = new PhysicsBody(sceneShapes[2], 1.0f);
				sceneBodies[numBody].position = new Vector2(5.0f, 0.0f);
				sceneBodies[numBody].shapeIndex = 2;
				numBody++;
				...
				// Lock the movement for the direction = nomalization of Vector2(-1.0f, 2.0f)
				// Lock the movement for the direction = nomalization of Vector2(2.0f, 1.0f) weakly (orthonomal to Vector2(-1.0f, 2.0f))
				sceneJoints[numJoint].axis1Lim = (new Vector2(-1.0f, 2.0f)).Normalize();
            ...
            {
				sceneBodies[numBody] = new PhysicsBody(sceneShapes[2], 1.0f);
				sceneBodies[numBody].position = new Vector2(25.0f, 0.0f);
				sceneBodies[numBody].shapeIndex = 2;
				numBody++;
				...
				// Lock the movement for the direction = nomalization of Vector2(-1.0f, 2.0f)
				// It means that the movement is free for the direction = nomalization of Vector2(2.0f, 1.0f) (orthonomal to Vector2(-1.0f, 2.0f))
				// But this free movement is limited by the distance -10.0f <-> +10.0f
				sceneJoints[numJoint].axis1Lim = (new Vector2(-1.0f, 2.0f)).Normalize();
		...
		public override void InitScene ()
        {
            ...
            // Any Direction Slider Joint without distance limitation
            {
				...
				body.Position = new Vector2(-15.0f, 0.0f);
				...
				// Lock the movement for the direction = nomalization of Vector2(-1.0f, 2.0f)
				// It means that the movement is free for the direction = nomalization of Vector2(2.0f, 1.0f)
				joint.Axis1Lim = (new Vector2(-1.0f, 2.0f)).Normalize();
				joint.Axis2Lim = new Vector2(0.0f, 0.0f);
				joint.AngleLim =1;
			}
            ...
            {
				PhysicsBody body = AddBody(SceneShapes[2], 1.0f);
				body.Position = new Vector2(5.0f, 0.0f);
				...
				// Lock the movement for the direction = nomalization of Vector2(2.0f, 1.0f) weakly (orthonomal to Vector2(-1.0f, 2.0f))
				joint.Axis1Lim = (new Vector2(-1.0f, 2.0f)).Normalize();
				joint.Axis2Lim = (new Vector2(2.0f, 1.0f)).Normalize() * 0.01f;
				joint.AngleLim =1;
			}	
            ...
            {
				PhysicsBody body = AddBody(SceneShapes[2], 1.0f);
				body.Position = new Vector2(25.0f, 0.0f);
				...
				// But this free movement is limited by the distance -10.0f <-> +10.0f
				joint.Axis1Lim = (new Vector2(-1.0f, 2.0f)).Normalize();
				joint.Axis2Lim = (new Vector2(2.0f, 1.0f)).Normalize();
