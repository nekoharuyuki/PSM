...
// [Tips and notice]
//
// This sample shows the way to create slider joint.
//
// Vertical slider joint (with & without limitation)
// Horizontal slider joint (with & without limitation)
//
// Horizontal slider joint will become a little slippy compared with vertial slider joint.
// In the case that you want to make horizontal slider joint stop finally, there is a solution
// to add air_frition to rigid body for stability.
//
...
namespace Physics2DSample
{
    ...
    public class SliderJointScene : PhysicsScene
	{	
		...
		public override void InitScene ()
        {
            ...
            // Vertical slider joint without distance limitation
            {
				...
				// Make the vertical motion free (Y direction)
			...
			// Vertical slider joint with distance limitation
            {
				sceneBodies[numBody] = new PhysicsBody(sceneShapes[2], 1.0f);
				...
				// Make the vertical motion free (Y direction)
				// In addition to this, added the following distance limitation
				// Please notice that axis2Lim should be set when there is a distance limitation
				sceneJoints[numJoint].axis1Lim = new Vector2(1.0f, 0.0f);
			...
			// Horizontal slider joint without distance limitation
            {
				sceneBodies[numBody] = new PhysicsBody(sceneShapes[2], 1.0f);
				...
				// Make the horizontal motion free (X direction)
				sceneJoints[numJoint].axis1Lim = new Vector2(0.0f, 0.0f);
				sceneJoints[numJoint].axis2Lim = new Vector2(0.0f, 1.0f);
				sceneJoints[numJoint].angleLim =1;
				
				// Slider joint is a little slippy
				// If you want to rigid body stop finally, it is a solution to give airFriction to rigid body,
				// and please try to use the following setting
				// bodyB.airFriction = 0.1f;
				
				numJoint++;
			}
			
			// Horizontal slider joint with distance limitation
            {
				sceneBodies[numBody] = new PhysicsBody(sceneShapes[2], 1.0f);
				...
				// Make the horizontal motion free (X direction)
				// In addition to this, added the following distance limitation
				// Please notice that axis1Lim should be set when there is a distance limitation
				sceneJoints[numJoint].axis1Lim = new Vector2(1.0f, 0.0f);	
		...
		public override void InitScene ()
        {
			...
			// Horizontal slider joint without distance limitation
            {
				...
				// Make the horizontal motion free (X direction)
				joint.Axis1Lim = new Vector2(0.0f, 0.0f);
				joint.Axis2Lim = new Vector2(0.0f, 1.0f);
				joint.AngleLim =1;
