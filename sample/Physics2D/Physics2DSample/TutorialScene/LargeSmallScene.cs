...
//
// Same mass and different length gives different feelings for the rendering 
//
...
// [Tips and notice]
//
// Different scale of length with same mass of two rigid sceneBodies (boxes)
// 
// Two boxes have same velocity actually.
// But the velocity of box seems faster than the velocity of box of large scale's one.
//
// When the rendering scale for small scale's simulation is changed to give the same kind of looking,
// it gives the feeling that the velocity of the small scale's one is faster though actual velocities are same.
//
...
namespace Physics2DSample
{
    ...
    public class LargeSmallScene : PhysicsScene
	{	
		...
		public override void InitScene ()
        {
			...
			// half scale size compared with sceneShapes[2]
			Vector2 half_box_width = new Vector2(1.0f, 1.0f);
            sceneShapes[3] = new PhysicsShape(half_box_width);
			
            Vector2 start_goal_bar = new Vector2(12.0f, 1.0f);
            sceneShapes[4] = new PhysicsShape(start_goal_bar);
			...
			// Large scale for length
			// But box has same mass
            {
                ...
                // Start bar
                {
                    ...
                    sceneBodies[numBody].position = new Vector2(-20.0f, 20.0f);
			...
			// Create dynamic rigid sceneBodies
			// Small scale for length
			// But box has same mass
			//
			// When you scale up just for rendering,
			// it seems that box is moving faster compared with the above case
			// even though the velocity is same actually compared with the above case
            {
		...
		public override void InitScene ()
        {
			...
			// half scale size compared with sceneShapes[2]
			Vector2 half_box_width = new Vector2(1.0f, 1.0f);
            AddBoxShape(half_box_width);
			
            Vector2 start_goal_bar = new Vector2(12.0f, 1.0f);
            AddBoxShape(start_goal_bar);
			...
			// But box has same mass
            {
                ...
                // Start bar
                {
                    ...
                    body.Position = new Vector2(-20.0f, 20.0f);
                }
