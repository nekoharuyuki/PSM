...
//
// The way to define the characteristic of the scene
//
...
// [Tips and notice]
//
// The folloiwng parameter determine the characteristic of the scene (normal, slippy, bumppy ... etc)
//
...
//
// It is not easy to say what kind of combination is suitable for your scene.
// Please try to use Change_SceneProperty(...) to change the characteristic of the scene.
...
namespace Physics2DSample
{
    ...
    public class ScenePropertyScene : PhysicsScene
	{	
		...
		// This saves the number of scene characteristic property
		private int scene_prop_num = 0;
			
        public ScenePropertyScene()
        {
		...
		private void Change_SceneProperty(int prop)
		{
			switch(prop)
			{
				case 0:
					// Very bumppy scene because of no friction for tangent direction
					penetrationRepulse = 0.5f;
					penetLimit = 0.0f;
					tangentFriction = 0.1f;
					this.restitutionCoeff = 0.90f;
				
					Console.WriteLine("Very bummpy scene");
				
					break;
				
				case 1:	
					// Very slippy scene because of no friction for tangent direction
					penetrationRepulse = 0.2f;
					penetLimit = 0.03f;
					tangentFriction = 0.0f; // this is a point of the characteristic
					this.restitutionCoeff = 0.0f;
				
					Console.WriteLine("Very slippy scene");
				
					break;
			
				case 2:
					// This is a usal setting, relatively stable simulation without bumppy behavior
		         	penetrationRepulse = 0.2f;
					penetLimit = 0.03f;
					tangentFriction = 0.3f;
					this.restitutionCoeff = 0.0f;
		...
		public override void InitScene ()
        {
            ...
            base.InitScene();
			sceneName = "ScenePropertyScene";
			
			// Set the scene property
			// Please check Change_SceneProperty(...)
			Change_SceneProperty(scene_prop_num);
            ...
            // Create dynamic rigid sceneBodies
            {
                // Create a dynamic rigid body whose shape is box (1.0 Kg)
                {
					...
					sceneBodies[numBody].angularVelocity = 5.0f;
					sceneBodies[numBody].shapeIndex = 2;
                    numBody++;
                }
				
				
				// Sphere is more unstable compared with Box or Convex shape
				// because it is easy for sphere to continue rolling.
				//
        ...
        public override void KeyboardFunc(GamePadButtons button)
        {
            switch (button)
            {
                case GamePadButtons.Square:
					scene_prop_num = (scene_prop_num + 1)%3;
					Change_SceneProperty(scene_prop_num);
                    break;
                default:
                    break;
