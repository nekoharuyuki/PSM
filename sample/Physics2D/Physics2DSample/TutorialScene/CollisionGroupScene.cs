...
//
// The way to use (collision & group) filter property of the rigid body together
//
...
// [Tips and notice]
//
// By using (collision & group) filter, you can cancel collision detection for some combinations.
// Please refer collision filter sample and group filter sample at first.
// When you use both (collision & group) filter together, both conditions are well considered inside broad phase.
//
// [Physics2DEngine/Broadphase.cs]
// if ((ob1.collisionFilter & ob2.collisionFilter) != 0) return false; (cancel collision)
// if ((ob1.groupFilter & ob2.groupFilter) == 0) return false;  (cancel collision)
//
...
namespace Physics2DSample
{
    ...
    public class CollisionGroupScene : PhysicsScene
	{	
		...
		public override void InitScene ()
        {
			...
			// Box vs Sphere - no collision
			// Box vs Convex - colllision 
			...
			for(int i=0; i<2; i++)
			{
				...
				sceneBodies[numBody].groupFilter = (1 << 1);
				numBody++;
			}
			...
			// Sphere vs Convex - no colllision 
			...
			for(int i=0; i<2; i++)
			{
				...
				if(i==0)
					...
					sceneBodies[numBody].collisionFilter = (1 << 2);				
				sceneBodies[numBody].groupFilter = (1 << 2);
				numBody++;
			}
			
			// Convex vs Box - collision
			// Convex vs Sphere - no collision
			// Convex vs Convex - colllision 
			...
			for(int i=0; i<2; i++)
			{
				...
				if(i==0)
					...
					sceneBodies[numBody].collisionFilter = (1 << 2);				
				sceneBodies[numBody].groupFilter = ((1 << 1) | (1 << 3));
				numBody++;
			}
