...
//
// The way to set the center of the joint
//
...
// [Tips and notice]
//
// It is not necessary that the center of the joint equals to the center of rigid body.
...
namespace Physics2DSample
{
    ...
    public class NonCenterJointScene : PhysicsScene
	{	
		...
		public override void InitScene ()
        {
			...
			// 
			// Please notice that the center of the joint is far awary from the center of the object,
			// the joint will become unstable because of the numerical error.
			//
			
            // Ball Joint with Center Position
            {
           ...
           // Ball Joint with Edge Position
            {
				...
				// The center of joint is the edge position of bodyB
				sceneJoints[numJoint] = new PhysicsJoint(bodyA, bodyB, bodyB.position - new Vector2(bodyB.width.X, 0), (uint)0, (uint)numBody-1);
			...
			// Ball Joint with the position out of object
            {
				sceneBodies[numBody] = new PhysicsBody(sceneShapes[2], 1.0f);
				sceneBodies[numBody].position = new Vector2(30.0f, 0.0f);
				sceneBodies[numBody].shapeIndex = 2;
				...
				// The center of joint is the edge position of bodyB
				sceneJoints[numJoint] = new PhysicsJoint(bodyA, bodyB, bodyB.position - new Vector2(bodyB.width.X, 0) - new Vector2(2.0f, 0.0f), (uint)0, (uint)numBody-1);
           ...
           // Ball Joint with Edge Position
            {
				...
				// The center of joint is the edge position of bodyB
				PhysicsJoint joint = AddJoint(bodyA, bodyB, bodyB.Position - new Vector2(bodyB.width.X, 0));
			...
			// Ball Joint with the position out of object
            {
				...
				// The center of joint is the edge position of bodyB
				PhysicsJoint joint = AddJoint(bodyA, bodyB, bodyB.Position - new Vector2(bodyB.width.X, 0) - new Vector2(2.0f, 0.0f));
