...
//
// Compound Rigid Body Scene --- 2D Rigid Body (Compound Rigid Body）Creation
//
...
namespace Physics2DSample
{
	...
	public class CompoundScene : PhysicsScene
    {
		...
		public override void InitScene ()
		{
            ...
            // Create compound rigid sceneBodies
            for(int i=0; i<5; i++)
			{
                // Combine 3 rigid sceneBodies(index0, index1, index2 in ascending order) into one
                uint index0, index1, index2;

				Vector2 pos = new Vector2(i*10 - 10, 0);

                // Create a box-shaped dynamic rigid body
				{
					sceneBodies[numBody] = new PhysicsBody(sceneShapes[2], 1.0f);
					sceneBodies[numBody].position = new Vector2(-box_width.X, 0.0f) + pos;
                ...
                // Create a sphere-shaped dynamic rigid body
				{
					...
					sceneBodies[numBody].position = new Vector2(0.0f, box_width.Y) + pos;
					sceneBodies[numBody].rotation = 0;
                ...
                // Combine 3 rigid sceneBodies into one
				uint[] compound_combination = new uint[3]{index0, index1, index2};
				PhysicsBody.MergeBody(sceneBodies, compound_combination, 3, sceneShapes, compoundMap);
		...
		public override void InitScene ()
		{
			...
			{
                // Combine 3 rigid sceneBodies(index0, index1, index2 in ascending order) into one
                PhysicsBody[] mergeList =  {null, null, null};
                ...
                // Create a box-shaped dynamic rigid body
				{
					...
					mergeList[0].Position = new Vector2(-box_width.X, 0.0f) + pos;
					mergeList[0].Rotation = 0.0f;
				}

                // Create a box-shaped dynamic rigid body
				{
					mergeList[1] = AddBody(SceneShapes[2], 1.0f);
					mergeList[1].Position = new Vector2(box_width.X, 0.0f) + pos;
					mergeList[1].Rotation = PhysicsUtility.GetRadian(30.0f);
				}

                // Create a sphere-shaped dynamic rigid body
				{
					mergeList[2] = AddBody(SceneShapes[3], 1.0f);
					mergeList[2].Position = new Vector2(0.0f, box_width.Y) + pos;
					mergeList[2].Rotation = 0;
					mergeList[2].ColFriction = 0.01f;
				}
