...
namespace Sce.PlayStation
{
    namespace HighLevel.UI
    {
        ...
        {
            const float defaultMaxDistanceInch = 0.157f;
            ...
            public DoubleTapGestureDetector()
            {
                ...
                this.MaxNextPressDuration = 300.0f;
            ...
            protected internal override GestureDetectorResponse OnTouchEvent(TouchEventCollection touchEvents)
            {
                ...
                float distance = firstDownPos.Distance(currentPox);

                float elapsedTime = (float)(touchEvent.Time - this.previousTime).TotalMilliseconds;
                ...
                switch (touchEvent.Type)
                {
                    case TouchEventType.Down:
                        if (this.downCount > 0)
                        {
                            if ((distance <= this.MaxDistance) && (elapsedTime <= this.MaxNextPressDuration))
                            {
                                this.downCount++;
                                this.previousTime = touchEvent.Time;
                            }
                            else
                            {
                                OnResetState();
                        ...
                        {
                            this.downCount++;
                            this.firstDownPos = touchEvent.WorldPosition;
                            this.previousTime = touchEvent.Time;
                        }
                    ...
                    case TouchEventType.Up:
                        if (this.downCount == requiredDownCount)
                        {
                            if ((distance <= this.MaxDistance) && (elapsedTime <= this.MaxPressDuration))
                            {
                                if (this.DoubleTapDetected != null)
                                {
                                    DoubleTapEventArgs args = new DoubleTapEventArgs(this.TargetWidget, touchEvent.WorldPosition, touchEvent.LocalPosition);
                                    this.DoubleTapDetected(this, args);
                                }
