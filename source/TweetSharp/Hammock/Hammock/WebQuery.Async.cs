                               ...
                               TimeSpan duration, int resultCount)
        {
            using (stream = response.GetResponseStream())
            {
                ...
                try
                {
					...
					const int STREAMBUF_SIZE = 10000;
					const int STREAM_SIZE = 4;
					const int CRLF_SIZE = 2;
					byte[] StreamBuf = new byte[STREAMBUF_SIZE];
					byte[] StreamSize = new byte[STREAM_SIZE];
					byte[] CrLf = new byte[CRLF_SIZE];
					StreamBuf = Enumerable.Repeat<byte>(0x00, STREAMBUF_SIZE).ToArray();
					
					stream.Read(StreamSize, 0, STREAM_SIZE);
					stream.Read(CrLf, 0, CRLF_SIZE);
					
					String str = System.Text.Encoding.UTF8.GetString(StreamSize);
					int size = int.Parse(str);
					stream.Read(StreamBuf, 0, size);
					
					while(StreamBuf[count] != 0)
                    {
						byteAsInt = StreamBuf[count];
						count++;
                        ...
                        if (nextChar[0] == streamResultDelimiter)
                        {
							...
							//StreamBuf = Enumerable.Repeat<byte>(0x00, STREAMBUF_SIZE).ToArray();
							StreamSize = Enumerable.Repeat<byte>(0x00, STREAM_SIZE).ToArray();
							CrLf = Enumerable.Repeat<byte>(0x00, CRLF_SIZE).ToArray();
							
							stream.Read(StreamSize, 0, STREAM_SIZE);
							stream.Read(CrLf, 0, CRLF_SIZE);
							
							str = System.Text.Encoding.UTF8.GetString(StreamSize);
							...
							{
								StreamBuf = Enumerable.Repeat<byte>(0x00, STREAMBUF_SIZE).ToArray();
								stream.Read(StreamBuf, 0, STREAMBUF_SIZE);
								break;
							}
                        }
                    }
